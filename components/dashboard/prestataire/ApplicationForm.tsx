...
const ApplicationForm:FC<{task: TaskProps}> = ({task}) => {
    ...
    const {applyTotask, application:[application], getTaskApplication, loading} = useApplication();
    ...
    const applicationRangeBarColor: {EN_ATTENTE:string, ACCEPTEE: string, REJETEE:string} = {
        ...
        ACCEPTEE: 'linear-gradient(to right, rgb(51, 211, 51) 100%, #fff 1%)',
        REJETEE: 'red'
    }
  ...
  return (
    <div className='transition-transform duration-300 ease-in-out'>
        {
            loading && 
            <div className="w-full h-80 bg-gray-300 animate-pulse rounded-sm">

            </div>

        }
        {    !loading && task &&
            <form onSubmit={handleSubmit(onSubmit, onError)} className='border opacity border-gray-200 md:border-2 p-5 bg-white'>
